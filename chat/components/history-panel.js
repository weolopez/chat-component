class HistoryPanel extends HTMLElement {
  ...
  updateContent() {
    ...
    this._chatHistory.forEach(chat => {
      ...
      let preview = previewMessage.length > 50 ? previewMessage.substring(0, 50) + '...' : previewMessage;
      //if  preview is an object, convert to string
      if (typeof preview !== 'string') {
        preview = preview[0] ? preview[0].text : JSON.stringify(preview);
      }
      ...
      historyItem.innerHTML = `
        ...
        <div class="history-item-date">${chat.messages.length}</div>
      `;
