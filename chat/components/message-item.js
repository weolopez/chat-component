class MessageItem extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['role', 'content', 'message', 'thinking', 'final_answer', 'timestamp', 'is-latest', 'image-url'];
  }
  ...
  get thinking() {
    return this.getAttribute('thinking') || '';
  }
  set thinking(value) {
  ...
  get final_answer() {
    return this.getAttribute('final_answer') || '';
  }
  set final_answer(value) {
    this.setAttribute('final_answer', value);
  }
  ...
  render() {
    ...
    let final_answer = this.final_answer;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .thinking-process {
          margin-bottom: 10px;
          background-color: var(--thinking-bg, #e0e0e0);
          border-radius: var(--border-radius, 8px);
          padding: 8px 12px;
          font-size: 0.85rem;
          color: var(--text-color, #2A2A2A);
            ...
            ${isAssistant ? `
              ${thinking ? `
                <details class="thinking-process">
                  ...
                  <pre class="thinking-content">${this.markdownToHtml(thinking)}</pre>
                </details>
              ` : ''}
              ${final_answer ? `<div class="final-answer-content">${this.markdownToHtml(final_answer)}</div>` : ''}
