    ...
    //TODO: if message.content is not a string, handle it appropriately
      ...
      if (messageType !== 'string') {
        ...
        if (typeof text !== 'string') {
          console.error('Message content should be a string:', JSON.stringify(message));
      ...
      let memory = await this.getRelevantMemories(memoryItem.text, 5) 
      if (memory.length == 0) {
          return;
      }
  ...
  async getRelevantMemories(query, limit = 5) {
    try {
      ...
      results = results.filter(m => {
        if (m.relevance === undefined || m.relevance === null) {
          if (m.similarity !== undefined) {
            m.relevance = m.similarity;
          } else {
            console.warn('Memory relevance is undefined, skipping:', m);
            return true;
          }
        }
        let kept = m.relevance < 0.78;
        if (!kept) console.warn(`Memory relevance too high, skipping (${m.relevance}):`, m.text);
        return kept;
      });
